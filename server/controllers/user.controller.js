...
export async function createUser(req, res) {
  ...
  const { firstName, lastName, username, email, role, divisions, status } =
    parseResult.data;
  ...
  try {
    ...
    return sendResponse(res, {
      ...
      message:
        "User created successfully. A password has been sent to the user's email.",
    });
  } catch (err) {
    if (err.code === 11000) {
...
export async function createUserByInstructor(req, res) {
  ...
  try {
    ...
    return sendResponse(res, {
      ...
      data: publicUser(populated),
      message:
        "User created and enrolled in bootcamp successfully. Password sent to email.",
    });
  } catch (err) {
    if (err.code === 11000) {
      const field = Object.keys(err.keyPattern || {})[0] || "field";
